...
void pipeline_t::dispatch() {
   ...
   for (i = 0; i < dispatch_width; i++) {
      ...
      PAY.buf[index].AL_index = REN->dispatch_inst(PAY.buf[index].C_valid, //dest_valid
                                                   PAY.buf[index].C_log_reg, //logical register
                                                   PAY.buf[index].C_phys_reg, //physical register
                                                   load_flag,              //load
                                                   store_flag,             //store
                                                   branch_flag,            //branch
                                                   amo_flag,               //amo
      ...
      // FIX_ME #8 BEGIN
      //check if the instruction has a source register
      // if yes, get the readiness from renamer
      // else alway make it ready
      ...
      if (PAY.buf[index].A_valid) D_ready = REN->is_ready(PAY.buf[index].D_phys_reg);
      else D_ready = true;
      ...
      // FIX_ME #9 BEGIN
      //decide whether to clear destination's ready bit 
      ...
      case SEL_IQ:
         ...
         IQ.dispatch(
            index,                                     //index
            DISPATCH[i].branch_mask,                   //branch_mask
            PAY.buf[index].lane_id,                    //lane_id
            PAY.buf[index].A_valid,                    //A_valid
            A_ready,                                   //A_ready
            PAY.buf[index].A_phys_reg,                 //A_tag
            PAY.buf[index].B_valid,                    //B_valid
            B_ready,                                   //B_ready
            PAY.buf[index].B_phys_reg,                 //B_tag
            PAY.buf[index].D_valid,                    //D_valid
            D_ready,                                   //D_ready
            PAY.buf[index].D_phys_reg,                 //D_tag
            inum_valid,                                //inum_valid
